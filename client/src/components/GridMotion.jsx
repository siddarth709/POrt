...
export default function GridMotion({ items = [] }) {
  ...
  return (
    <section id="grid-motion" className="grid-motion-shell relative min-h-screen py-24 px-4 sm:px-8 bg-[#060710] overflow-hidden isolate">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            >
              ...
              <span className="font-mono text-xs text-indigo-300 uppercase tracking-widest">Interactive Bento Vault</span>
            </motion.div>
        ...
        {/* ── Bento Grid Showcase ── */}
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 auto-rows-[260px]">
          {filteredItems.slice(0, 9).map((item, index) => {
            // Determine bento card size pattern
            ...
            return (
              <motion.div
                ...
                className={`group relative rounded-3xl border border-white/10 bg-slate-900/40 backdrop-blur-xl overflow-hidden cursor-pointer p-6 flex flex-col justify-between transition-all duration-500 hover:border-indigo-500/50 hover:shadow-[0_0_40px_rgba(99,102,241,0.25)] ${gridClass}`}
              >
                ...
                <div className="relative z-10 flex items-center justify-between">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-mono tracking-widest uppercase bg-black/70 border border-white/15 text-indigo-300 backdrop-blur-md">
                    <ShieldCheck size={11} className="text-indigo-400" />
                    {item.title ? item.title.split(' ')[0] : 'VAULT ITEM'}
                  </span>

                  <span className="p-2 rounded-full bg-white/10 border border-white/15 text-slate-300 group-hover:text-white group-hover:bg-indigo-600/80 group-hover:border-indigo-400/60 transition-all duration-300">
                ...
                <div className="relative z-10">
                  <h3 className={`font-bold text-white tracking-tight line-clamp-2 mb-1 group-hover:text-indigo-200 transition-colors ${isLarge ? 'text-2xl sm:text-3xl' : 'text-lg'}`}>
                    {item.title || 'System Specimen'}
                  </h3>
                  <p className="text-xs font-mono text-slate-400 flex items-center gap-1">
                    <Eye size={12} className="text-indigo-400" /> Click to inspect architecture
                  </p>
                </div>
              </motion.div>
            );
          ...
          <motion.div
            ...
            onMouseMove={handleMouseMove}
            className="md:col-span-1 md:row-span-1 group relative rounded-3xl border border-indigo-500/30 bg-gradient-to-br from-indigo-950/40 via-slate-900/60 to-slate-950/80 backdrop-blur-xl p-6 flex flex-col justify-between overflow-hidden"
          >
            <div className="flex items-center justify-between z-10">
              <span className="flex items-center gap-2 text-xs font-mono text-indigo-300 uppercase">
                <Terminal size={14} className="text-emerald-400" /> SYSTEM HEALTH
              </span>
            ...
            <div className="z-10 my-auto">
              <div className="text-3xl font-mono font-black text-white">99.98<span className="text-emerald-400 text-sm">%</span></div>
              <p className="text-xs text-slate-400 font-mono mt-1">Model Inference Precision</p>
            </div>

            <div className="z-10 flex items-center justify-between text-[11px] font-mono text-slate-400 border-t border-white/10 pt-3">
        ...
        {selectedCard && (
          ...
          >
            <motion.div
              ...
              className="relative max-w-4xl w-full max-h-[90vh] rounded-3xl overflow-hidden border border-white/20 bg-slate-900/95 shadow-[0_0_80px_rgba(99,102,241,0.3)] flex flex-col"
            >
              {/* Drawer Header */}
              <div className="p-6 border-b border-white/10 flex items-center justify-between bg-slate-900/60">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">{selectedCard.title || 'System Specimen'}</h3>
                    <span className="text-xs font-mono text-indigo-400 uppercase tracking-wider">Detailed Vault Artifact</span>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col lg:flex-row gap-6">
                <div className="lg:w-3/5 rounded-2xl overflow-hidden border border-white/10 bg-black flex items-center justify-center">
                  <img
                ...
                <div className="lg:w-2/5 flex flex-col justify-between gap-4">
                  <div className="space-y-4">
                    <div className="p-4 rounded-xl border border-white/10 bg-white/[0.03] space-y-2">
                      <span className="text-xs font-mono text-slate-400 uppercase">SPECIFICATION</span>
                    ...
                    <div className="space-y-2">
                      <span className="text-xs font-mono text-slate-400 uppercase">TAGS & ARCHITECTURE</span>
                      <div className="flex flex-wrap gap-2">
                        {['PyTorch', 'CUDA', 'Distributed', 'Deep Learning'].map((tag) => (
                          <span key={tag} className="px-3 py-1 rounded-full text-xs font-mono border border-white/10 bg-white/5 text-indigo-300">
                            {tag}
                          </span>
                        ))}
                      </div>
